...
const CodeBlock: React.FC<CodeBlockProps> = ({ language, code }) => {
  ...
  return (
    <div className="my-4 rounded-2xl border border-sky-500/30 bg-[#070a14] overflow-hidden shadow-2xl transition-all duration-200 hover:border-sky-400/50 group">
      {/* Code Header Bar with Copy to Clipboard Button & Icon Indicator */}
      <div className="flex items-center justify-between px-4 py-2.5 bg-[#0b1020]/90 backdrop-blur-md border-b border-white/10 text-xs text-slate-400">
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex items-center gap-1.5 pl-2 border-l border-white/10">
            ...
            <span className="uppercase text-[11px] font-bold tracking-wider text-sky-300 font-mono">
              {language || 'CODE'}
            </span>
        ...
        {/* Injected 'Copy to Clipboard' Button with Icon Indicator */}
        <button
          id={`copy-clipboard-btn-${language || 'code'}-${Date.now().toString(36).slice(4)}`}
          onClick={handleCopy}
          className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-[11px] font-mono font-medium transition-all duration-200 cursor-pointer shadow-md active:scale-95 ${
            copied
              ? 'bg-emerald-500/20 text-emerald-300 border-emerald-400 shadow-[0_0_12px_rgba(52,211,153,0.3)]'
              : 'bg-white/5 hover:bg-sky-500/15 text-slate-300 hover:text-sky-200 border-white/10 hover:border-sky-400/40'
          }`}
        ...
        >
          {copied ? (
            <>
              ...
              <span className="text-emerald-300 tracking-wide font-semibold">COPIED TO CLIPBOARD!</span>
            </>
          ) : (
            <>
              <Copy className="w-3.5 h-3.5 text-sky-400 transition-transform group-hover:scale-110" />
              <span className="tracking-wide">Copy to Clipboard</span>
            </>
          )}
        </button>
      </div>
